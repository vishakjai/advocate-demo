...
local gitalyApdexSlowMethods = [
  ...
  // This RPC can be slow depending on the range of commits we have to walk. It's
  // primarily consumed asynchronously when a user visits the repository page of a
  // fork and thus not on the critical path.
...
local defaultSatisfiedThreshold = 0.5;
local defaultToleratedThreshold = 1;
...
local grpcServiceApdex(
  baseSelector,
  satisfiedThreshold=defaultSatisfiedThreshold,
  toleratedThreshold=defaultToleratedThreshold,
...

{
  ...
  defaultSatisfiedThreshold:: defaultSatisfiedThreshold,
  defaultToleratedThreshold:: defaultToleratedThreshold,
