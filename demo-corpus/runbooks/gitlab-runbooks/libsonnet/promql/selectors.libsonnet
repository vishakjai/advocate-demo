...
local serializeHashItem(label, value) =
  if std.isString(value) || std.isNumber(value) then
    ...
    serializeItems(label, '=', std.toString(value))
    ...
    error '%s is invalid for a selector with label %s' % [value, label];

{
  ...
  merge(a, b)::
    ...
    else if std.isObject(a) && std.isObject(b) then
      objects.nestedMerge(a, b)
    else
      a + b,
