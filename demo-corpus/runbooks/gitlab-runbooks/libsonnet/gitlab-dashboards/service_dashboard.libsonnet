...
local overviewDashboard(
  ...
  expectMultipleSeries=metricsCatalog.getService(type).isShardLevelMonitored(),
      ) =
  ...
  local dashboard =
    ...
    .addTemplateIf(
      !metricsCatalogServiceInfo.serviceIsStageless && std.objectHas(environmentStageSelectorHash, 'stage'),
      templates.stage
    )
    .addTemplateIf(
      metricsCatalogServiceInfo.isShardLevelMonitored(),
      templates.shard(type, current=defaultShard)
    )
    .addAnnotationIf(
