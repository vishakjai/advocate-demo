...
local titleRowWithPanels(title, panels, collapse, startRow, repeat=null) =
  local titleRow = grafana.row.new(title=title, collapse=collapse, repeat=repeat) { gridPos: { x: 0, y: startRow, w: 24, h: 1 } };
  if collapse then
...

{
  ...
  rowGrid(rowTitle, panels, startRow, rowHeight=10, collapse=false, repeat=null)::
    local panelRow = self.singleRow(panels, rowHeight=rowHeight, startRow=(startRow + 1));
    titleRowWithPanels(rowTitle, panelRow, collapse, startRow, repeat=repeat),
