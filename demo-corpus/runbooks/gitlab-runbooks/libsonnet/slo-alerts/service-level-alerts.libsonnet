...
local commonAnnotations(serviceType, aggregationSet, metricName) =
  ...
  {
    runbook: '%(serviceType)s/#alerts' % formatConfig,  // We can do better than this
...
local calcRequiredOpsRate(windowDuration, minimumSamplesForMonitoring, minimumOpsRateForMonitoring) =
  local windowMinOpsRate = minimumOpRate.calculateFromSamplesForDuration(windowDuration, minimumSamplesForMonitoring);
  if windowMinOpsRate == null && minimumOpsRateForMonitoring == null then
    null
  else if windowMinOpsRate == null then
    minimumOpsRateForMonitoring
  else if minimumOpsRateForMonitoring == null then
    windowMinOpsRate
  else
    std.max(windowMinOpsRate, minimumOpsRateForMonitoring);
...
local apdexAlertsForSLI(
  ...
  minimumOpsRateForMonitoring=null,  // Minimum ops rate regardless of the window size, see docs/metrics-catalog/service-level-monitoring.md
  ...
  [
    {
      ...
      expr: multiburnExpression.multiburnRateApdexExpression(
        ...
        requiredOpRate=calcRequiredOpsRate(windowDuration, minimumSamplesForMonitoring, minimumOpsRateForMonitoring),
