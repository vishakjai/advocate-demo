...
local kubeStateMetricTypeDescriptors = {
  ...
  container: {
    ...
    applicableToService(service):: true,
    metrics: [
  ...
  node: {
    ...
    keyLabel: 'node',
    extraLabels: ['cloud_google_com_gke_nodepool'],
    applicableToService(service):: service.hasDedicatedKubeNodePool(),
    metrics: [
...
local generateEnrichedLabelsForServices(services, descriptorType, extraSelector) =
  local descriptor = kubeStateMetricTypeDescriptors[descriptorType];
  local filteredServices = std.filter(descriptor.applicableToService, services);
  if std.length(filteredServices) > 0 then
    local serviceSelector = { type: { oneOf: [s.type for s in filteredServices] } };
...
local generateEnrichedRulesForServices(services, extraSelector) =
  {
    name: 'kube-state-metrics-recording-rules: enriched label recording rules',
    interval: '1m',
    rules:
      generateEnrichedLabelsForServices(services, 'container', extraSelector) +
      generateEnrichedLabelsForServices(services, 'pod', extraSelector) +
      generateEnrichedLabelsForServices(services, 'hpa', extraSelector) +
      generateEnrichedLabelsForServices(services, 'node', extraSelector) +
      generateEnrichedLabelsForServices(services, 'ingress', extraSelector) +
      generateEnrichedLabelsForServices(services, 'deployment', extraSelector),
  };
...

{
  ...
  groupsWithFilter(filterFn, extraSelector={}):
    ...
    then []
      ...
      // We generate the key metrics selector labels (For example `kube_node_labels:labeled`)
      // in a group per service in all environments.
      // But the `:labeled` metrics that join those key metrics, we generate in a single group for
      // all services in dedicated. For GitLab.com we generate them per service as well, to make sure they
      // arrive in the right monitoring tenant.
      // That is why we have this split and the double iteration here.
      [
        generateKubeSelectorRulesForService(service, extraSelector)
        for service in filteredServices
      ] + [
        generateEnrichedRulesForServices(filteredServices, extraSelector),
      ],
}
