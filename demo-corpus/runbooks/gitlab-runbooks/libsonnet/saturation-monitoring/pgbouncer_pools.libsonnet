...
local pgbouncerAsyncPool(tag, role) =
  resourceSaturationPoint({
    title: 'Postgres Async Urgent (Sidekiq) %s Connection Pool Utilization per Node' % [role],
    severity: 's4',
    ...
    description: |||
      pgbouncer async urgent connection pool utilization per database node, for %(role)s database connections.
    ...
    query: |||
      (
        avg_over_time(pgbouncer_pools_server_active_connections{user="gitlab", database=~"gitlabhq_production_sidekiq_urgent.*", %(selector)s}[%(rangeInterval)s]) +
        avg_over_time(pgbouncer_pools_server_testing_connections{user="gitlab", database=~"gitlabhq_production_sidekiq_urgent.*", %(selector)s}[%(rangeInterval)s]) +
        avg_over_time(pgbouncer_pools_server_used_connections{user="gitlab", database=~"gitlabhq_production_sidekiq_urgent.*", %(selector)s}[%(rangeInterval)s]) +
        avg_over_time(pgbouncer_pools_server_login_connections{user="gitlab", database=~"gitlabhq_production_sidekiq_urgent.*", %(selector)s}[%(rangeInterval)s])
      )
      / on(%(aggregationLabels)s) group_left()
      sum by (%(aggregationLabels)s) (
        label_replace(
          avg_over_time(pgbouncer_databases_pool_size{name=~"gitlabhq_production_sidekiq_urgent.*", %(selector)s}[%(rangeInterval)s]),
          "database", "$1", "name", "(.*)"
        )
      )
    |||,
