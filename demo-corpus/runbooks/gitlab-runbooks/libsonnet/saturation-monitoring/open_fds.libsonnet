...

{
  open_fds: resourceSaturationPoint({
    ...
    appliesTo: metricsCatalog.findServicesExcluding(
      excluding=['cloud-sql', 'gitlab-static', 'kube', 'nat', 'cloudflare', 'http-router', 'ci-runners'] + metricsCatalog.findServicesWithTag(tag='runway-managed-redis') + metricsCatalog.findServicesWithTag(tag='runway-managed-postgres')
    ),
    description: |||
