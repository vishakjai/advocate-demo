local nestedMerge(target, override) =
  assert std.isObject(target) : 'target needs to be an object but was %s' % [target];
  assert std.isObject(override) : 'override needs to be an object but was %s' % [override];
  target + {
    [attr]:
      local current = std.get(target, attr, {});
      if std.isObject(override[attr]) && std.isObject(current)
      then nestedMerge(current, override[attr])
      else override[attr]
