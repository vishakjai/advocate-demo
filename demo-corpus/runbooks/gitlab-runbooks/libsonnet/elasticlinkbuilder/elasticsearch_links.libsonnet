...
local defaultIndexCatalog = import 'index_catalog.libsonnet';
...
local buildElasticDiscoverSearchQueryURL(index, indexCatalog, filters=[], luceneQueries=[], timeRange=grafanaTimeRange, sort=[], extraColumns=[]) =
  ...
  ic.kibanaEndpoint + ic.discoverEndpoint + '?_a=' + rison.encode(applicationState) + globalState(timeRange);

local buildElasticLineCountVizURL(index, indexCatalog, filters, luceneQueries=[], splitSeries=false, timeRange=grafanaTimeRange) =
  local ic = indexCatalog[index];
  ...
  ic.kibanaEndpoint + ic.visualizeEndpoint + '/create?type=line&indexPattern=' + indexCatalog[index].indexPattern + '&_a=' + rison.encode(applicationState) + globalState(timeRange);
...
local buildElasticTableCountVizURL(index, indexCatalog, filters, luceneQueries=[], splitSeries=false, timeRange=grafanaTimeRange, extraAggs=[], orderById='1') =
  local ic = indexCatalog[index];
  local aggs =
    [
      {
  ...
  ic.kibanaEndpoint + ic.visualizeEndpoint + '/create?type=table&indexPattern=' + indexCatalog[index].indexPattern + '&_a=' + rison.encode(applicationState) + globalState(timeRange);


local buildElasticLineTotalDurationVizURL(index, indexCatalog, filters, luceneQueries=[], latencyField, splitSeries=false) =
  local ic = indexCatalog[index];
  ...
  ic.kibanaEndpoint + ic.visualizeEndpoint + '/create?type=line&indexPattern=' + indexCatalog[index].indexPattern + '&_a=' + rison.encode(applicationState) + globalState(grafanaTimeRange);

local buildElasticLinePercentileVizURL(index, indexCatalog, filters, luceneQueries=[], optionalLatencyField, splitSeries=false, percentile) =
  local ic = indexCatalog[index];
  local latencyField = if optionalLatencyField == null then
    indexCatalog[index].defaultLatencyField
  else
    optionalLatencyField;
...

{
  ...
  // TODO: Change consumers to use linkGenerator instead of these legacy wrapper functions.
  buildElasticDiscoverSearchQueryURL:: function(index, filters=[], luceneQueries=[], timeRange=grafanaTimeRange, sort=[], extraColumns=[])
    buildElasticDiscoverSearchQueryURL(index, defaultIndexCatalog, filters, luceneQueries, timeRange, sort, extraColumns),

  buildElasticLineCountVizURL: function(index, filters, luceneQueries=[], splitSeries=false, timeRange=grafanaTimeRange,)
    buildElasticLineCountVizURL(index, defaultIndexCatalog, filters, luceneQueries, splitSeries=splitSeries, timeRange=timeRange),

  buildElasticLinePercentileVizURL: function(index, filters, luceneQueries=[], field=null, splitSeries=false, percentile=95)
    buildElasticLinePercentileVizURL(index, defaultIndexCatalog, filters, luceneQueries, field, splitSeries, percentile),

  buildElasticTableCountVizURL: function(index, filters, luceneQueries=[], splitSeries=false, timeRange=grafanaTimeRange, extraAggs=[], orderById='1')
    buildElasticTableCountVizURL(index, defaultIndexCatalog, filters, luceneQueries, splitSeries, timeRange, extraAggs, orderById),

  buildElasticTableFailureCountVizURL: function(index, filters, luceneQueries=[], splitSeries=false, timeRange=grafanaTimeRange)
    buildElasticTableCountVizURL(index, defaultIndexCatalog, filters + defaultIndexCatalog[index].failureFilter, luceneQueries, splitSeries, timeRange),

  linkGenerator(indexCatalog, defaultPrometheusLabelMappings):: {

    // Given an index, and a set of filters, returns a URL to a Kibana discover module/search
    buildElasticDiscoverSearchQueryURL: function(index, filters=[], luceneQueries=[], timeRange=grafanaTimeRange, sort=[], extraColumns=[])
      buildElasticDiscoverSearchQueryURL(index, indexCatalog, filters, luceneQueries, timeRange, sort, extraColumns),

    // Search for failed requests
    buildElasticDiscoverFailureSearchQueryURL: function(index, filters=[], luceneQueries=[], timeRange=grafanaTimeRange, sort=[], extraColumns=[])
    ...
    buildElasticDiscoverSlowRequestSearchQueryURL: function(index, filters=[], luceneQueries=[], slowRequestSeconds=null, timeRange=grafanaTimeRange, extraColumns=[])
      local ic = indexCatalog[index];
    ...
    buildElasticLineCountVizURL: function(index, filters, luceneQueries=[], splitSeries=false, timeRange=grafanaTimeRange,)
      buildElasticLineCountVizURL(index, indexCatalog, filters, luceneQueries, splitSeries=splitSeries, timeRange=timeRange),

    buildElasticLineFailureCountVizURL: function(index, filters, luceneQueries=[], splitSeries=false, timeRange=grafanaTimeRange)
    ...
    buildElasticTableCountVizURL: function(index, filters, luceneQueries=[], splitSeries=false, timeRange=grafanaTimeRange, extraAggs=[], orderById='1')
      buildElasticTableCountVizURL(index, indexCatalog, filters, luceneQueries, splitSeries, timeRange, extraAggs, orderById),

    buildElasticTableFailureCountVizURL: function(index, filters, luceneQueries=[], splitSeries=false, timeRange=grafanaTimeRange)
      buildElasticTableCountVizURL(index, indexCatalog, filters + indexCatalog[index].failureFilter, luceneQueries, splitSeries, timeRange),
    ...
    */
    buildElasticLineTotalDurationVizURL: function(index, filters, luceneQueries=[], field=null, splitSeries=false)
      local fieldWithDefault = if field == null then
        indexCatalog[index].defaultLatencyField
      else
        field;
      buildElasticLineTotalDurationVizURL(index, indexCatalog, filters, luceneQueries, fieldWithDefault, splitSeries=splitSeries),

    // Given an index, and a set of filters, returns a URL to a Kibana percentile visualization
    buildElasticLinePercentileVizURL: function(index, filters, luceneQueries=[], field=null, splitSeries=false, percentile=95)
      buildElasticLinePercentileVizURL(index, indexCatalog, filters, luceneQueries, field, splitSeries=splitSeries, percentile=percentile),

    // Returns true iff the named index supports request graphs (some do not have a concept of 'requests')
    indexSupportsRequestGraphs: function(index)
      !std.objectHas(indexCatalog[index], 'requestsNotSupported'),

    // Returns true iff the named index supports failure queries
    indexSupportsFailureQueries: function(index)
      std.objectHas(indexCatalog[index], 'failureFilter'),

    // Returns true iff the named index supports latency queries
    indexSupportsLatencyQueries: function(index)
      std.objectHas(indexCatalog[index], 'defaultLatencyField'),

    indexHasSlowRequestFilter: function(index)
    ...
    */
    getMatchersForPrometheusSelectorHash: function(index, selectorHash)
