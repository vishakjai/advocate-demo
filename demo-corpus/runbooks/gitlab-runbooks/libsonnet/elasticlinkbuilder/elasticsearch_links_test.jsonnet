...
local elastic = (import 'elasticlinkbuilder/elasticsearch_links.libsonnet').linkGenerator(indexCatalog, defaultPrometheusLabelMappings);
...
test.suite({
  testBuildElasticDiscoverSearchQueryURL: {
    ...
    expect: joinMulti(|||
      ...
      _a=(columns:!(json.class,json.queue,json.meta.project,json.job_status,json.queue_duration_s,json.duration_s),filters:!((meta:(index:'AWNABDRwNDuQHTm2tH6l'),query:(match:(json.shard:(query:throttled,type:phrase))))),index:'AWNABDRwNDuQHTm2tH6l')&
  ...
  testBuildElasticDiscoverSearchQueryURLWithExtraColumns: {
    ...
    expect: joinMulti(|||
      https://log.gprd.gitlab.net/app/kibana#/discover?
      _a=(columns:!(json.class,json.queue,json.meta.project,json.job_status,json.queue_duration_s,json.duration_s,json.username),filters:!(),index:'AWNABDRwNDuQHTm2tH6l')
    |||),
  },
  ...
  testGetMatchersForPrometheusSelectorHashTranslation: {
    ...
    expect: [
      {
        meta: { key: 'json.meta.feature_category', type: 'phrases', params: ['helm_chart_registry', 'package_registry'] },
        query: { bool: { minimum_should_match: 1, should: [{ match_phrase: { 'json.meta.feature_category': 'helm_chart_registry' } }, { match_phrase: { 'json.meta.feature_category': 'package_registry' } }] } },
      },
    ],
  },
