...
local missingFeatureCategoryWithEmptyFeatureCategorySelector(missingLabels, selector) =
  ...
  std.member(missingLabels, 'feature_category') && std.isString(featureCategoryRe) && std.member(std.split(featureCategoryRe, '|'), '');
...
local resolveRecordingRuleFor(metricName, requiredAggregationLabels, selector, duration) =
  ...
  local durationWithRecordingRule = if std.startsWith(duration, '$__') then '5m' else duration;
