local queryBuilder = import './query-builder.libsonnet';
...
local errorBudgetTimeSpent(queries, range, selectors, aggregationLabels, ignoreComponents) =
  |||
    (
      (
         1 - %(ratioQuery)s
  ...
  ||| % {
    ratioQuery: queries.combinedRatio(selectors, aggregationLabels, ignoreComponents),
...
local errorBudgetTimeRemaining(queries, slaTarget, range, selectors, aggregationLabels, ignoreComponents) =
  |||
    # The number of seconds allowed to be spent in %(range)s
    %(budgetSeconds)s
    -
  ...
  ||| % {
    range: range,
    budgetSeconds: utils.budgetSeconds(slaTarget, range),
    timeSpentQuery: errorBudgetTimeSpent(queries, range, selectors, aggregationLabels, ignoreComponents),
  };
...

{
  init(slaTarget, range): {
    local aggregationSet = gitlabMetricsConfig.aggregationSets.serviceComponentStageGroupSLIs,
    local queries = queryBuilder.build(aggregationSet, range),

    errorBudgetRatio: queries.combinedRatio,
    errorBudgetApdexRatio: queries.apdexRatio,
    errorBudgetErrorRatio: queries.errorRatio,

    errorBudgetTimeSpent(selectors, aggregationLabels=[], ignoreComponents=true):
      errorBudgetTimeSpent(queries, range, selectors, aggregationLabels, ignoreComponents),
    ...
    errorBudgetTimeRemaining(selectors, aggregationLabels=[], ignoreComponents=true):
      errorBudgetTimeRemaining(queries, slaTarget, range, selectors, aggregationLabels, ignoreComponents),
