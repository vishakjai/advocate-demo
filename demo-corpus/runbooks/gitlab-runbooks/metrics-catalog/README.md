...
metricsCatalog.serviceDefinition({
  ...
  team: 'team_name'         // The team name from `teams.yml`, this will be the team that alerts are routed to
  ...
  serviceLevelIndicators: { // A map of SLIs for this service. The name is defined by the key
    workhorse: {            // Defines a service called `workhorse`
     ...
     apdex: histogramApdex(                                                // See https://prometheus.io/docs/practices/histograms/#apdex-score
      ...
      // Team can be overridden for per SLI. But will default to the value set for the service
      team: 'overridden_team',
...
1. **SLO violation alerts** for each SLI for apdex and error ratios. More about it in [alert-routing](../docs/uncategorized/alert-routing.md)
1. **Weighted Average SLA** caclulation as described in [the handbook](https://about.gitlab.com/handbook/engineering/monitoring/)
1. **Grafana Dashboards** for each service with SLI overview panels and detailed drilldown panels. See [the web Overview in Grafana](https://dashboards.gitlab.net/d/web-main) for an example
