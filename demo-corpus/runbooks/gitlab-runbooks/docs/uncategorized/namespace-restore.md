...
The [congregate tool](https://gitlab.com/gitlab-org/professional-services-automation/tools/migration/congregate) is a migration tool maintained by the Professional Services team.

Installation and configuration is [described here](https://gitlab.com/gitlab-org/professional-services-automation/tools/migration/congregate/-/blob/master/docs/full_setup.md#configuring).

After initializing the tool with `congregate init`, you provide configuration settings in the `data/congregate.conf` file. All the fields are [described in the template](https://gitlab.com/gitlab-org/professional-services-automation/tools/migration/congregate/-/blob/master/congregate.conf.template).
Personal Access tokens should be generated on the source and destination instances. The token values are then entered into the config as Base64-encoded to obfuscate them. Obfuscation can be done using `congregate obfuscate`. Example config:
...
After this, if the tokens and IP's are configured correctly, running `congregate list` should succeed.  If it doesn't, check [Congregate's logs](https://gitlab.com/gitlab-org/professional-services-automation/tools/migration/congregate/-/blob/af88620cfa0b4530b9fc71ec519beae1978b7864/runbooks/airgapped-migration-usage.md#checking-logs) in `/opt/congregate/data/logs`, or watch the logs on the source server for the reason why (`sudo gitlab-ctl tail gitlab-rails`)
