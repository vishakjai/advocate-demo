...
```
/chatops gitlab run deploycmd CMD_NAME BASE_ROLE_NAME [--ENVIRONMENT] [--skip-haproxy] [--no-check]
```
...
```
/chatops gitlab run deploycmd hostname base_stor --skip-haproxy
```
...
```
/chatops gitlab run deploycmd hostname base_stor --no-check --production
```
