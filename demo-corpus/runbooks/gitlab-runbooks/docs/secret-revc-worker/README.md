...
* [secret-revc-worker](https://console.cloud.google.com/run/detail/us-east1/secret-revc-worker/logs?project=gitlab-runway-production)

<!-- END_MARKER -->

## Summary

Secret Revocation Worker (`secret-revc-worker`) is a Runway-based workload/deployment that is a part of the [Secret Revocation Service](https://gitlab.com/gitlab-org/security-products/secret-detection/secret-revocation-service).

It consumes messages published from the corresponding [API workload](../secret-revocation/README.md) to a Google PubSub [topic](https://gitlab.com/gitlab-org/security-products/secret-detection/secret-revocation-service#pubsub_topic) via a [subscription](https://gitlab.com/gitlab-org/security-products/secret-detection/secret-revocation-service#pubsub_subscription). When a message is received, it is parsed and the appropriate handler is used to send a revocation request to the partner API.
