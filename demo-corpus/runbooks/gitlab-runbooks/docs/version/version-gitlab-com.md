...
```
gcloud container clusters get-credentials version-staging-gke --region us-east1 --project version-staging-eae216
gcloud container clusters get-credentials version-prod-gke --region us-east1 --project version-prod-674cf6
```
...
- CloudSQL instance `cloudsql-7ea7` (the 4 character suffix is necessary for terraform and will change with future deployments)
...
The review and staging deployments share the `version-staging-eae216` GCP project. The Kubernetes cluster is similar, but the databases are deployed as pods, so there is no CloudSQL instance

## Database

The production database resides in a regional (`us-east1`) HA CloudSQL instance. The name will change when rebuilt. To retrieve the current name:

```bash
gcloud --project version-prod-674cf6 beta sql instances list
```
...
This GCP project and the infrastructure components in it are managed by the [gitlab-com/gl-infra/gitlab-services](https://gitlab.com/gitlab-com/gl-infra/gitlab-services) project.  Any infrastructure changes to the environment or K8s cluster should be made as an MR there.  Changes will be applied automatically via CI jobs when the MR is merged.  `version-prod` and `version-staging` are represented as [Environments](https://gitlab.com/gitlab-com/gl-infra/gitlab-services/-/environments) in that project.
