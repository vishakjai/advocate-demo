    ...
    queries=[
      {
        ...
        query: 'sum(gitlab_runner_jobs{environment=~"$environment", stage=~"$stage", shard=~"${shard:pipe}"})',
      },
      {
        legendFormat: 'Number of {{state}} machines',
        query: 'sum by (state) (gitlab_runner_autoscaling_machine_states{environment=~"$environment", stage=~"$stage", shard=~"${shard:pipe}", state=~"idle|used|creating|removing", executor="docker+machine"})',
      },
      {
        legendFormat: 'Number of existing machines',
        query: 'sum(gitlab_runner_autoscaling_machine_states{environment=~"$environment", stage=~"$stage", shard=~"${shard:pipe}", state=~"idle|used|creating|removing", executor="docker+machine"})',
      },
      {
        legendFormat: 'Limit utilization',
        query: 'sum(gitlab_runner_jobs{environment=~"$environment", stage=~"$stage", shard=~"${shard:pipe}"}) / sum(gitlab_runner_limit{shard=~"${shard:pipe}"}) ',
      },
    ],
  ) + {
    seriesOverrides+: [
