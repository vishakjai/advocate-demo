...
.addPanels(
  layout.grid([
    networkPanel('Fe (HAProxy) Registry Data Transfer / 24h', 'fqdn=~"^haproxy-registry-[0-9].*"'),
    networkPanel('Fe (HAProxy) Data Transfer / 24h', 'fqdn=~"^haproxy-[0-9].*"'),
    networkPanel('Fe Pages (HAProxy) Data Transfer / 24h', 'fqdn=~"^haproxy-pages-[0-9].*"'),
