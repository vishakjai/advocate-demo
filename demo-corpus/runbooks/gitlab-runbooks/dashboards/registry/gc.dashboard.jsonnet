...
.addPanels(
  layout.grid([
    ...
    .addTarget(
      promQuery.target(
        'sum(avg by (query_name) (last_over_time(registry_database_rows{query_name=~"gc_.*_review_queue", environment="$environment"}[$__interval])))',
      )
    ),
    statPanel.new(
      title='Pending Tasks',
    ...
    .addTarget(
      promQuery.target(
        'sum(avg by (query_name) (last_over_time(registry_database_rows{query_name=~"gc_.*_review_queue_overdue", environment="$environment"}[$__interval])))',
      )
    )
  ...
  layout.grid(
    [
      panel.timeSeries(
        ...
        query='avg by (query_name) (registry_database_rows{query_name=~"gc_.*_review_queue", environment="$environment"})',
      ...
      panel.timeSeries(
        ...
        query='avg by (query_name) (registry_database_rows{query_name=~"gc_.*_review_queue_overdue", environment="$environment"})',
        legendFormat='{{ query_name }}',
        intervalFactor=1,
        yAxisLabel='Count'
      ),
