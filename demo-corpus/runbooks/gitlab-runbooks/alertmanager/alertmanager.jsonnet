...
local slackChannels = [
  ...
  { name: 'fleet_alerts_slack_channel', channel: 'f_fleet_alerts' },
  { name: 'global_search_slack_channel', channel: 'g_global_search' },
];
          ...
          slackActionButton(  // runbook
            ...
            url=|||
              ...
              {{- else if ne (index .Alerts 0).Annotations.runbook "" -}}
                https://runbooks.gitlab.com/{{ (index .Alerts 0).Annotations.runbook -}}
              {{- else -}}
                https://runbooks.gitlab.com/uncategorized/alerts-should-have-runbook-annotations/
              {{- end -}}
            |||
          ),
...
local routingTree = Route(
  ...
  [
    ...
    Route(
      ...
      group_by=groupByTypeAndPager,
    ),
  ] + [
    /* Route cell alerts to incident.io */
    Route(
      receiver='incidentio',
      continue=true,
      repeat_interval='2h',
      ...
      group_by=['alertname', 'tenant', 'tenant_type'],
    ),
  ] + [
    /*
    ...
    // Route elasticsearch shard sidekiq paging alerts to #g_global_search channel
    Route(
      receiver='global_search_slack_channel',
      continue=true,
      matchers={
        env: 'gprd',
        type: 'sidekiq',
  ...
  [
    // Route fleet_management (type=fleet) alerts to #f_fleet_alerts
    // This needs to go before the blackhole route to ensure it hits the slack channel instead
    Route(
      receiver='fleet_alerts_slack_channel',
      continue=true,
      matchers={ type: 'fleet' },
    ),
  ] +
  [
    ...
    Route(
      receiver='database_operations_pagerduty',
      matchers={
        pager: 'database_operations_pagerduty',
      },
      group_by=groupByType,
      continue=false,
    ),
...
local inhibitRules() =
  ...
  +
  // Don't fire alerts for 1h or 6h window when the other is already firing for the same `type`, `env`, `component`
  // This will also silence regional, per-node or per-shard alerts if there was already one firing.
  [
    {
      target_matchers: selectors.alertManagerMatchers({ slo_alert: 'yes' }),
      source_matchers: selectors.alertManagerMatchers({ slo_alert: 'yes' }),
      equal: ['env', 'type', 'component'],
    },
  ];
